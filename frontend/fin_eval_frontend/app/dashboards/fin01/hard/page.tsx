...
export default function Fin01HardDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const judgmentData = await loadJudgmentData('fin_o1', 'hard');
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Advanced financial reasoning performance</p>
        </div>

        <DashboardStats stats={stats} colorScheme="purple" />
        <DashboardTable data={data} modelName="Fin-01-14B - Hard" />
      </div>
    </div>
  );
}
